// Vendored from tlock-js 0.9.0 (commit 17d817ee259e79381111dd75009b0f022c39ace3, MIT), src/age/stream-cipher.ts.
// ArcSeal changes: an empty plaintext is sealed as one empty final chunk and `open` refuses a payload without a
// final chunk, as the age spec and Go age require (upstream emitted no chunk, so a payload cut to its nonce
// decrypted to an empty plaintext and an encrypted empty file could not be opened by tle).
...
export class STREAM {
    static seal(plaintext: ui8a, privateKey: ui8a): Uint8Array {
        ...
        const chunks = Math.max(1, Math.ceil(plaintext.length / CHUNK_SIZE))
    ...
    static open(ciphertext: ui8a, privateKey: ui8a): Uint8Array {
        ...
        const lastChunkLength = ciphertext.length - (chunks - 1) * ENCRYPTED_CHUNK_SIZE
        if (chunks === 0 || lastChunkLength < TAG_SIZE) throw new Error("Missing or truncated final chunk")
