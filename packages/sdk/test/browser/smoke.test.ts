// Browser smoke test (PRD 8.2, 10.1): same harness as packages/tlock/test/browser. Runs the `vite build` CLI on
// index.html + main.ts (which imports the SDK entry, so viem, zod, pino's browser build and @arcseal/tlock) with no
// config file (no polyfill plugins, no target override) in a clean production env, serves the output from a local
// static server and runs it in headless Playwright Chromium. Offline: the page only loads local files.
...
beforeAll(async () => {
  outDir = mkdtempSync(join(tmpdir(), 'arcseal-sdk-smoke-'));
...
describe('browser smoke (Vite build + headless Chromium)', () => {
  ...
  it('seals and unseals votes with a committed beacon in Chromium with zero console errors', async () => {
    ...
    expect(log).toContain('ok   sealVote then unsealVote with a committed beacon');
    expect(log).toContain('ok   a wrong beacon and a truncated ciphertext unseal to null');
